	...
	<HEAD>
		...
		<SCRIPT LANGUAGE="JavaScript">
			...
			function submit(){
				...
				var reloadIntervalMsec = document.getElementById("reloadIntervalMsec2");
				reloadIntervalMsec.value = document.getElementById("reloadIntervalMsec").value;
				
				var reloadActive = document.getElementById("reloadActive2");
				reloadActive.value = document.getElementById("reloadActive").value;
	...
	<body>
		...
		<FORM name="logtreeform" method="POST">
			<table class="tableView" id="fileSettings">
				...
				<tr>
					...
					<script>writeTD('rowdata');</script>
						<div style="margin-left: 10px;margin-right: 10px;">
							<script>
								writeInputFieldWithId("reloadIntervalMsec","reloadIntervalMsec", "%value reloadIntervalMsec%", 10, "EDIT");
							</script>
						</div>
				...
				<tr>
					...
					<script>writeTD('rowdata');</script>
						<div style="margin-left: 10px;margin-right: 10px;">
							<input type="checkbox" id="reloadActive" name="reloadActive" value="true" %ifvar reloadActive equals('true')%checked%endifvar% />
						</div>
					</td>
					<script>writeTD('row-l');</script>
						<div style="margin-left: 10px;margin-right: 10px;">
							Requires manual package reload or IS restart for a change to take effect.
						</div>
						<p/>
						<div style=" margin-left: 10px;">
							If active, every package start/reload creates another thread watching the file. 
							These threads can only be stopped by a complete JVM restart (does not happen by IS restart via UI).
						</div>
					</td>
					<script>swapRows();</script>
				</tr>
			...
			<FORM name="logform" action="settingsLogging.dsp" method="POST">
				...
				<INPUT type="hidden" name="reloadIntervalMsec" id="reloadIntervalMsec2">
				<INPUT type="hidden" name="reloadActive" id="reloadActive2">
